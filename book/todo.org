...
- [X] in output from pdflatex, one cannot see which cells are input cells and which are output cells.
  - similar in html
  - maybe this works: output is printed in black, input is generally coloured?
    - [X] Update text where it explains input and output cells (search for "In")
- test CI locally
